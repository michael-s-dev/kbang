...
void Downloader::openDownload() {
    if (!m_filePath.isEmpty()) {
        ...
        {
            QFile::remove(QCoreApplication::applicationDirPath() + "/" + filename );
        }
        QFile::copy( m_filePath , QCoreApplication::applicationDirPath() + "/" + filename  );
        // Open
        if (url.startsWith ("/"))
            url = "file://" + QCoreApplication::applicationDirPath() + "/" + filename;
        else
            url = "file:///" + QCoreApplication::applicationDirPath() + "/" + filename;

        QProcess::startDetached( "update.exe" , QStringList() << "/O");
