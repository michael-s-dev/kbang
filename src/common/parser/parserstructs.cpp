...
{
    QByteArray bytes = QByteArray::fromBase64(node->getFirstChild()->text().toLatin1());
...
void ActionPlayCardData::read(XmlNode* node)
{
    ...
    if (!node->attribute("card-card2").isNull()) {
           type = PLAYCARD_CARD_CARD;
           targetCardId = node->attribute("target-card-id").toInt();
           targetCard2Id = node->attribute("target-card2-id").toInt();
            qDebug() << "card" << targetCardId << " player " << targetCard2Id;
    }else if (!node->attribute("card-player").isNull()) {
           type = PLAYCARD_CARD_PLAYER;
           targetCardId = node->attribute("target-card-id").toInt();
           targetPlayerId = node->attribute("target-player-id").toInt();
            qDebug() << "card" << targetCardId << " player " << targetPlayerId;
...
void ActionPlayCardData::write(QXmlStreamWriter* writer) const
{
    ...
    case PLAYCARD_HAND:
        ...
        break;
    case PLAYCARD_CARD_CARD:
        writer->writeAttribute("card-card2", QString::number(1));
        writer->writeAttribute("target-card-id", QString::number(targetCardId));
        writer->writeAttribute("target-card2-id", QString::number(targetCard2Id));
        break;
    case PLAYCARD_CARD_PLAYER:
        writer->writeAttribute("card-player", QString::number(1));
        writer->writeAttribute("target-card-id", QString::number(targetCardId));
        writer->writeAttribute("target-player-id", QString::number(targetPlayerId));
        break;
    }
