...
bool GameActionManager::onCardClicked(CardWidget* cardWidget)
{
    ...
    case STATE_MAIN:
        ...
        if ( /*cardWidget->pocketType() == POCKET_HAND && for catch table cards as well*/ cardWidget->ownerId() == mp_game->playerId())
...
{
    if (cardWidget->pocketType() == POCKET_HAND) { // Cards in hand event
...
void GameActionManager::playWithCards()
{
    if (m_cardSelection.size() == 1) {
        ...
        if ( (card2->pocketType() != POCKET_TABLE && card2->ownerId() != mp_game->playerId() ) ||
             (card2->type() == Card::Character && card2->ownerId() == mp_game->playerId() ))
           mp_game->serverConnection()->playCardWithCardAndPlayer(mp_activeCard->cardData().id , card->cardData().id , card2->ownerId());
         else
           mp_game->serverConnection()->playCardWithCardAndCard(mp_activeCard->cardData().id , card->cardData().id , card2->cardData().id);
