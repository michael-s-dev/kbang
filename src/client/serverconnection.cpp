...
// PlayCard with card on player
void ServerConnection::playCardWithCardAndCard(int cardId , int otherCardId , int targetCardId)
{
    if (mp_parser == 0)
        return;
    ActionPlayCardData actionPlayCardData;
    actionPlayCardData.playedCardId = cardId;
    actionPlayCardData.type = ActionPlayCardData::PLAYCARD_CARD_CARD;
    actionPlayCardData.targetCardId = otherCardId;
    actionPlayCardData.targetCard2Id = targetCardId;
    mp_parser->actionPlayCard(actionPlayCardData);
 }
void ServerConnection::playCardWithCardAndPlayer(int cardId , int otherCardId , int targetPlayerId)
{
    if (mp_parser == 0)
        return;
    ActionPlayCardData actionPlayCardData;
    actionPlayCardData.playedCardId = cardId;
    actionPlayCardData.type = ActionPlayCardData::PLAYCARD_CARD_PLAYER;
    actionPlayCardData.targetCardId = otherCardId;
    actionPlayCardData.targetPlayerId = targetPlayerId;
    mp_parser->actionPlayCard(actionPlayCardData);
 }
...
void ServerConnection::useAbility(QList<int> cards , int targetPlayerId )
{
    if (mp_parser == 0)
        return;
    ActionUseAbilityData actionUseAbilityData;
    actionUseAbilityData.type = ActionUseAbilityData::TypeCardsPlayer;
    actionUseAbilityData.targetCardsId = cards;
    actionUseAbilityData.targetPlayerId = targetPlayerId;
    mp_parser->actionUseAbility(actionUseAbilityData);
