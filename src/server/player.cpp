...
bool Player::canPlayGreen() const
{
    foreach(PlayingCard* c, m_table) {
       c->setGreenReady(1);
    }
...
{
    if (this->characterType() == CHARACTER_TEQUILA_JOE)
    modifyLifePoints(2, this);
    else
...
{
    m_predrawChecks.removeOne(priority); // @m removeAll -> remove one , double dynamite fix
}
...
void Player::predrawCheck(int checkId)
{
    ...
    if (checkId == 0)
        return;
    int index = m_predrawChecks.lastIndexOf(checkId);  // @m indexOf -> lastIndexOf , double dynamite fix
    Q_ASSERT(index > 0);
