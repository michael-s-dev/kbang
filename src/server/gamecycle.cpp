...
void GameCycle::playCard(Player* player, PlayingCard* card, PlayingCard* targetCard , PlayingCard* targetCard2)
{
    m_contextDirty = 0;
    if (player != mp_requestedPlayer)
        throw BadPlayerException(mp_currentPlayer->id());
    ...
    player->character()->playCard(card, targetCard , targetCard2);
    sendRequest();
}
...
{
    if ( player->characterType() == CHARACTER_SEAN_MALLORY)
        return 0;
