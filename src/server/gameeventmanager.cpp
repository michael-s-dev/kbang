...
void GameEventManager::onPlayerPlayCard(Player* p, PlayingCard* card , PocketType pocketFrom)
{
    foreach(Handler* h, m_handlers) {
        h->handler->onPlayerPlayCard(p->publicView(), card , pocketFrom);
    }
}

void GameEventManager::onPlayerPlayCard(Player* p, PlayingCard* card, Player* targetPlayer , PocketType pocketFrom)
{
    foreach(Handler* h, m_handlers) {
        h->handler->onPlayerPlayCard(p->publicView(), card, targetPlayer->publicView() , pocketFrom);
    }
}

void GameEventManager::onPlayerPlayCard(Player* p, PlayingCard* card, PlayingCard* targetCard ,PocketType pocketFrom)
{
    ...
    foreach(Handler* h, m_handlers) {
        if (targetCard->pocket() == POCKET_HAND && targetCard->owner() == h->player || h->isSupervisor) {
            h->handler->onPlayerPlayCard(p->publicView(), card, targetCard, owner , pocketFrom);
        } else {
            h->handler->onPlayerPlayCard(p->publicView(), card, 0, owner , pocketFrom);
        }
    }
}
...
void GameEventManager::onPlayerRespondWithCard(Player* p, PlayingCard* card , PocketType pocketfrom)
{
    foreach(Handler* h, m_handlers) {
        h->handler->onPlayerRespondWithCard(p->publicView(), card , pocketfrom);
    }
}
