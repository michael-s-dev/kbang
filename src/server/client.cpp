...
void Client::onActionPlayCard(const ActionPlayCardData& actionPlayCardData)
{
    ...
    try {
        ...
        case ActionPlayCardData::PLAYCARD_CARD_CARD:{
            int targetCardId = actionPlayCardData.targetCardId;
            int targetCard2Id = actionPlayCardData.targetCard2Id;
            PlayingCard* card = mp_playerCtrl->card(targetCardId);
            PlayingCard* card2 = mp_playerCtrl->card(targetCard2Id);
            if (card == 0 || card2 == 0) {
                    qDebug(qPrintable(QString("[CLIENT]   Target card '%1' not exist!").arg(targetCardId)));
                    // @todo feedback
                    return;
                }
            mp_playerCtrl->playCard(playedCard, card , card2);
        }break;
        case ActionPlayCardData::PLAYCARD_CARD_PLAYER:{
            int targetCardId = actionPlayCardData.targetCardId;
            PlayingCard* card = mp_playerCtrl->card(targetCardId);
            int targetPlayerId = actionPlayCardData.targetPlayerId ;
            const PublicPlayerView* targetPlayer = mp_publicGameView->publicPlayerView(targetPlayerId);

            if (targetPlayer == 0 || card == 0 ) {
                qDebug(qPrintable(QString("[CLIENT]   Target player '%1' not exist!").arg(actionPlayCardData.targetPlayerId)));
                // @todo feedback
                return;
            }
            mp_playerCtrl->playCard(playedCard, card , targetPlayer);
        }break;
        }
    } catch (GameException& e) {
...
{
    if (mp_parser->isKeepAlive() == false  ) {
        onParserTerminated();
    }
...
void Client::onPlayerPlayCard(PublicPlayerView& player, const PlayingCard* card , PocketType pocketFrom)
{
    if (mp_parser == 0) return;
...
void Client::onPlayerPlayCard(PublicPlayerView& player, const PlayingCard* card, PublicPlayerView& target, PocketType pocketFrom)
{
    if (mp_parser == 0) return;
...
void Client::onPlayerPlayCard(PublicPlayerView& player, const PlayingCard* card, const PlayingCard* target, PublicPlayerView* targetPlayer , PocketType pocketFrom)
{
    if (mp_parser == 0) return;
...
void Client::onPlayerRespondWithCard(PublicPlayerView& player, const PlayingCard* card , PocketType pocketFrom)
{
    if (mp_parser == 0) return;
