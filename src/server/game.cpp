...
void Game::buryPlayer(Player* player, Player* causedBy)
{
    ...
    // Characters
    if (  m_state != GAMESTATE_FINISHED ){
        foreach(Player* p, this->playerList())
                {
                    if (p->characterType() == CHARACTER_HERB_HUNTER && p->isAlive() )
                    {
                       gameTable().playerDrawFromDeck(p, 2, 0);
                    }
                    if (p->characterType() == CHARACTER_GREG_DIGGER && p->isAlive()){
                        p->modifyLifePoints(2, 0);
                }
             }
      }
}
